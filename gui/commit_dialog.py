...
class Commit_dialog(QtWidgets.QWidget):
    ...
    def setupUi(self, commit_dialog):
            """the script dialog setup  """
        #main window
            commit_dialog.setObjectName("commit_dialog")
            commit_dialog.setWindowModality(QtCore.Qt.ApplicationModal)
            commit_dialog.resize(543, 466)
        #main window vertical layout
            self.verticalLayout = QtWidgets.QVBoxLayout(commit_dialog)
            self.verticalLayout.setObjectName("verticalLayout")
        #toolbox container
            self.toolbox = QtWidgets.QWidget(commit_dialog)
        ...
        #toolbox commit button
            self.commit_btn = QtWidgets.QPushButton(self.toolbox)
            self.commit_btn.setCursor(
                QtGui.QCursor(QtCore.Qt.PointingHandCursor))
            self.commit_btn.setObjectName("commit_btn")
            self.horizontalLayout.addWidget(self.commit_btn)
            self.commit_btn.clicked.connect(self.commit_script)
            self.verticalLayout.addWidget(self.toolbox)
        #loading label
            self.loading_label = QtWidgets.QLabel(commit_dialog)
            self.loading_label.setAlignment(QtCore.Qt.AlignCenter)
            self.loading_label.setObjectName("loading_label")
            self.loading_label.hide()
            self.verticalLayout.addWidget(self.loading_label)
        #loading container
            self.loading_container = QtWidgets.QWidget(commit_dialog)
        ...
        #input login container
            self.login_container = QtWidgets.QWidget(commit_dialog)
        ...
        #display text widget
            self.display_text = QtWidgets.QTextEdit(commit_dialog)
            self.display_text.setObjectName("display_text")
            self.display_text.setReadOnly(True)
            self.display_text.setPlaceholderText("The list of ips to commit: \n" + '\n'.join(self.ips))
            self.verticalLayout.addWidget(self.display_text)

            self.retranslateUi(commit_dialog)
            QtCore.QMetaObject.connectSlotsByName(commit_dialog)
    ...
    def retranslateUi(self, commit_dialog):
        ...
        self.loading_label.setText(_translate(
            "commit_dialog", "Executing the script"))
    ...
    def commit_script(self):
        ...
        self._thread.start()
        self.request.emit([self.ips, self.getInputs])
...
class Threaded(QtCore.QObject):
    ...
    def automate_config(self, args):
        ...
        self.device.commit(args[0], args[1], increment)
